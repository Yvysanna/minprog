...
void init_board(void)
{
    ...
    for(int i = 0, number = (d*d) - 1; i < d; i++){
        for(int j = 0; j < d; j++, number--){
            board[i][j] = number;
        }
    }

    // If odd number of tiles, switch last two to make puzzle solvable
    if (d % 2 == 0){
...
bool move(int tile)
{
    // Find coordinates of tile to be moved
    for (int i = 0; i < d; i++){
        for (int j = 0; j < d; j++){

            // When coordinate of tile is found:
            if (tile == board[i][j]){

                // Check if coordinate possible and empty
